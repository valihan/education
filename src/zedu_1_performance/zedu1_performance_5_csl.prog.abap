...
*    ENDLOOP.
    ...
    lt_xrev = VALUE #( FOR ls_data IN mt_data
                       USING KEY xreversing
                       WHERE ( xreversing = abap_true )
    ...
    TYPES: BEGIN OF ts_tabw,
             bwasl  TYPE tabw-bwasl,
             bwakon TYPE tabw-bwakon,
           END OF ts_tabw.
    DATA lt_tabw TYPE HASHED TABLE OF ts_tabw WITH UNIQUE KEY bwasl.
    SELECT bwasl, bwakon
      FROM tabw
      INTO TABLE @lt_tabw.
    IF sy-subrc <> 0.
    ...
    LOOP AT mt_data ASSIGNING FIELD-SYMBOL(<ls_data>).
      IF <ls_data>-xreversing = abap_false.
        IF <ls_data>-anbwa IS NOT INITIAL.
          ASSIGN lt_tabw[ bwasl = <ls_data>-anbwa ] TO FIELD-SYMBOL(<ls_tabw>).
          IF sy-subrc = 0.
